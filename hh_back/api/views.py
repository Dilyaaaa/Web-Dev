...
def companies_list(request):
    try:
        ...
        return JsonResponse({'Error status': str(e)}, status=400)
...
def company_details(request, id):
    try:
        ...
        return JsonResponse({'Error status': str(e)}, 200)
...
def company_vacancies_list(request, id):
    ...
    return JsonResponse(company_vacancies, safe=False, status=200)
...
def vacancies_list(request):
    ...
    if request.method == 'POST':
        ...
        return JsonResponse(new_vacancy.to_json(), safe=False, status=200)
